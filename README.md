...
For decision-making purposes, the raw data of $X$ is converted into a binary state based on the user's Desired Outcome Set ($X_{dn}$).
...
## 4. Data Methodology and Logic

### A. Simulation Logic (Monte Carlo)
...
### C. Flexibility of Scope ($X_{dn}$)
To increase flexibility, the Desired Outcome is treated as a function ($f(X_{dn})$). If a primary desired outcome ($X_{d1}$) is statistically impossible or fails, the range can be extended to a secondary desire ($X_{d2}$).
...
## 5. The Three-Part Thesis Journey

1.  **Part 1: Theoretical Application**
...
## 6. Software Framework: User Flow
